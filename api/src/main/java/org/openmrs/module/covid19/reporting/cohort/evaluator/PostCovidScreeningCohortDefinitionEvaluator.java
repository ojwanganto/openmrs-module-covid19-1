...
/**
 * Evaluator for post Covid screening cohort definition
 */
@Handler(supports = { PostCovidScreeningCohortDefinition.class })
public class PostCovidScreeningCohortDefinitionEvaluator implements EncounterQueryEvaluator {
	...
	public EncounterQueryResult evaluate(EncounterQuery definition, EvaluationContext context) throws EvaluationException {
		...
		String qry = "select encounter_id from kenyaemr_etl.etl_cca_post_covid_screening where date(visit_date) between date(:startDate) and date(:endDate);";
